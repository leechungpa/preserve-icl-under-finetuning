...
def attention(V, Q, K, Z):
    # Z: (B, N, D)
    _, N, D = Z.shape

    # Projections: (B, N, D)
    queries = torch.einsum('ij,BNj->BNi', Q, Z)
    keys    = torch.einsum('ij,BNj->BNi', K, Z)
    values  = torch.einsum('ij,BNj->BNi', V, Z)

    Attn = torch.einsum('BNd,BMd->BNM', queries, keys)
...
class Transformer_F(nn.Module):
    ...
    def __init__(self, n_layer, n_head, d, params_std=0.05):
        ...
        self.register_parameter('k_matrix', nn.Parameter(torch.zeros(n_layer, n_head, 1, d+2, d+2)))

        with torch.no_grad():
            self.v_matrix.normal_(0, params_std)
            self.q_matrix.normal_(0, params_std)
            self.k_matrix.normal_(0, params_std)
    ...
    def forward(self, Z):
        for i in range(self.n_layer):
            ...
            for j in range(self.n_head):
                ...
                Kij = self.k_matrix[i, j, 0, :, :]
                residues = residues + attention(Vij, Qij, Kij, Zi)
    ...
    def allparam(self):
        ...
        return torch.cat((self.v_matrix, self.q_matrix, self.k_matrix), dim=2)
...
def clip_and_step(model, optimizer, toclip, threshold=1.0):
    ...
    grad_k = model.k_matrix.grad

    grads = [g for g in (grad_v, grad_q, grad_k) if g is not None]
    if len(grads) == 0:
